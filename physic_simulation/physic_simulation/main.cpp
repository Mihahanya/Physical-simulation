...
int main()
{
    ...
    SoftContour fig(mass, jumpling, friction, elastic, resistance, connection_by_dist);
    ...
    VolumetricWall vw(center+vec2(-60, -320), vec2(100, 100), 0);
    scene.add(vw);
    ...
    while (window.isOpen())
    {
        ...
        if (Mouse::isButtonPressed(Mouse::Left) and !scene.pause) {
            ...
            fig.points[0].add_force(p*300.f - fig.points[0].vel*20.f);
